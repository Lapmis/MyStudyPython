...
def repeats(our_str):
    """
    Преобразует строку, добавляя справа от каждого символа количество его повторений в формате _n
    :param our_str: строка подлежащая подсчету
    :return: возвращает новую строку с поствиксом напротив каждого символа, в формате _n,где  n = количество повторений
    """
    new_str = ''
    for i in range(len(our_str)):
        point = 0
        point = our_str[:i + 1].count(our_str[i])
        new_str += '_'.join([our_str[i], str(point)])
    return new_str
