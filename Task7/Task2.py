...
class PersonInfo:
    ...
    def short_name(self):
        """
        Преобразовывает атрибут класса Строка из "Имя Фамилия" в формат "Фамилия И."
        :return: строку в формате "Фамилия И."
        """
        lst = self.name.split()
        return ' '.join([lst[1], lst[0][0]]) + '.'

    def path_deps(self):
        """
        Создает строку в котором указап путь подразделения в котором работает сотрудник
    ...
    def new_salary(self):
        """
        Расчет индексации зарплаты сотрудника по формуле: "1337 * Возраст * суммарное кол-во вхождений трех
        наиболее часто встречающихся букв из списка подразделений"
        :return: значение проиндексированной зарплаты
        """
        letters = ''.join(self.directory)
        letters_dict = {}
        for i in letters:
            letters_dict[i] = letters.count(i)
        lst = list(letters_dict.values())
        lst.sort(reverse=1)
        return 1337 * self.age * sum(lst[:3])
