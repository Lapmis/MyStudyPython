...
class RomanNums:
    ...
    def from_roman(self):
        """
        Метод преобразовывает римский формат отображения значения в арабское
        :return: значение числа в арабском формате
        """
        roman_dict = {'I': 1, 'V': 5, 'X': 10, 'L': 50, 'C': 100, 'D': 500, 'M': 1000}
        arabian_value = 0
        for i in range(0, len(self.str)):
            if i == 0 or roman_dict[self.str[i]] <= roman_dict[self.str[i - 1]]:
                arabian_value += roman_dict[self.str[i]]
            else:
                arabian_value += roman_dict[self.str[i]] - 2 * roman_dict[self.str[i - 1]]
        return arabian_value


    def is_palindrome(self):
        """
        Проверяет является ли число в римсклм формате палиндромом после преобразования значения в арабский формате
        :return: true если число является палиндромом, либо false если число не является палиндромом
        """
        num = RomanNums.from_roman(self)
